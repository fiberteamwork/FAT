...
Number.isFinite(customer.longitude)
    ...
    const rawFatData = customer.dataFatFull && typeof customer.dataFatFull === 'object'
        ...
        : {};
    const fatTypeValue = Object.keys(rawFatData).find(key => normalizeKey(key) === 'tipe fat');
    const fatType = String(
        (fatTypeValue ? rawFatData[fatTypeValue] : undefined) ??
        customer.tipeFat ??
        customer.fatType ??
        '-'
    ).trim() || '-';
