                    ...
                    try {
                        ...
                        try {
                            const buildCsvFromCustomers = (arr) => {
                                const headers = ['Label','Kota/Kabupaten','Nama Site','Keterangan','Tanggal','Latitude','Longitude'];
                                const lines = [headers.join(';')];
                                for (const it of (arr || [])) {
                                    const label = it.label || it.username || it.id || '';
                                    const city = it.city ?? '';
                                    const site = it.site ?? '';
                                    const ket = it.ket || it.status || '';
                                    ...
                                    const row = [label, city, site, ket, visitDate, lat, lon]
...
function updateChart(data) {
    ...
    data.forEach(c => {
        const type = String(c.dataFatFull?.['Tipe FAT'] || 'Tidak diketahui').trim() || 'Tidak diketahui';
    ...
    wardChart = new Chart(
            canvas,
            {
                ...
                options: {
                    responsive: true,
                    scales: {
                        x: { title: { display: true, text: 'Tipe FAT' } },
...
function setupCsvUpload() {
   ...
   uploadInput.addEventListener("change", function(event) {
       ...
       if (isXlsx) {
           // XLSX: baca sebagai arrayBuffer lalu parse via XLSXLite
           const reader = new FileReader();
           reader.onload = async function(evt) {
               try {
                   const buf = evt.target.result;
                   const rows = await xlsxRowsFromBuffer(buf);
                   console.log('[CUSTOMER_MAP] loading uploaded XLSX file', file.name, 'rows=', rows.length);
                   await loadData(rows);
                   try { alert('XLSX berhasil dimuat: ' + rows.length + ' baris.'); } catch (e) {}
               } catch (err) {
                   console.error('XLSX upload gagal:', err);
                   try { alert('XLSX upload gagal: ' + (err.message || err)); } catch (e) {}
               }
           };
           reader.onerror = function(evt) {
               console.error('Gagal membaca file XLSX', evt);
               try { alert('Gagal membaca file XLSX.'); } catch (e) {}
           };
           reader.readAsArrayBuffer(file);
           return;
       }
       // CSV: baca sebagai teks dan muat (tanpa menyimpan ke server)
       const reader = new FileReader();
       reader.onload = async function(evt) {
           ...
           if (typeof contents === "string") {
               ...
               try {
                   await loadData(contents);
                   try { alert('CSV berhasil dimuat.'); } catch (e) {}
               } catch (err) {
                   console.error('CSV upload gagal:', err);
                   try { alert('CSV upload gagal: ' + (err.message || err)); } catch (e) {}
               }
           }
       };
       reader.onerror = function(evt) {
           console.error('Gagal membaca file CSV', evt);
           try { alert('Gagal membaca file CSV.'); } catch (e) {}
       };
